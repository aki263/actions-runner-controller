...
check_os() {
    ...
    if [[ "$ID" != "ubuntu" ]]; then
        print_warning "This script is optimized for Ubuntu, you're running $PRETTY_NAME"
        print_info "Some package names might need adjustment..."
    fi
}
...
main() {
    print_header "Firecracker VM Setup for Ubuntu 24.04"
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --help|-h)
                ...
                echo ""
                echo "This script creates and starts a Firecracker VM with Ubuntu 24.04"
                echo "Must be run on Linux/Ubuntu (not macOS)"
                echo ""
                echo "Options:"
                ...
                echo ""
                echo "Examples:"
                echo "  $0                              # Default VM (1GB RAM, 2 CPUs)"
                echo "  $0 -m 2048 -c 4 -s 20G        # 2GB RAM, 4 CPUs, 20GB disk"
                echo "  $0 -k ./vmlinux-custom         # Use custom kernel"
                echo ""
                echo "Requirements:"
                echo "  - Linux with KVM support"
                echo "  - firecracker, debootstrap, qemu-utils"
                echo "  - sudo access for network setup"
                exit 0
                ;;
            *)
