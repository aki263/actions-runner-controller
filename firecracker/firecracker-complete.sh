...
check_dependencies() {
    # Allow skipping dependency checks via environment variable OR if running in container
    if [[ "${SKIP_DEPS:-false}" == "true" ]] || [[ -f "/.dockerenv" ]] || [[ "${container:-}" == "docker" ]]; then
        print_warning "Skipping dependency checks (container environment or SKIP_DEPS=true)"
        return 0
    fi
...
setup_workspace() {
    # In container environments, use the mounted volume path
    if [[ "${container:-}" == "docker" ]] || [[ -f "/.dockerenv" ]] || [[ "${FIRECRACKER_WORK_DIR:-}" != "" ]]; then
        WORK_DIR="${FIRECRACKER_WORK_DIR:-/app/firecracker-data}"
        print_info "Container mode: Using mounted work directory: ${WORK_DIR}"
    else
        # Traditional mode: use script directory
        WORK_DIR="${SCRIPT_DIR}/firecracker-data"
    fi
...
launch_vm() {
    ...
    # Setup instance
    local vm_id=$(echo "$runner_name" | tr '[:upper:]' '[:lower:]' | sha256sum | head -c 8)
    ...
    if [ "$use_host_bridge" = true ]; then
        ...
        if ! ip link show "$tap_device" >/dev/null 2>&1; then
            ...
            if ip tuntap add dev "$tap_device" mode tap 2>/dev/null; then
                print_info "✅ TAP device created: $tap_device"
            else
                print_error "Failed to create TAP device $tap_device"
                print_info "Debug: Checking if device exists now..."
                if ip link show "$tap_device" >/dev/null 2>&1; then
                    print_warning "TAP device exists despite creation error - continuing"
                else
                    exit 1
                fi
            fi
            
            if ip link set dev "$tap_device" master "$bridge_device" 2>/dev/null; then
                print_info "✅ TAP device attached to bridge: $bridge_device"
            else
                print_error "Failed to attach TAP device to bridge"
                exit 1
            fi
            
            if ip link set dev "$tap_device" up 2>/dev/null; then
                print_info "✅ TAP device brought up: $tap_device"
            else
                print_error "Failed to bring up TAP device $tap_device"
                exit 1
            fi
        else
            print_info "TAP device already exists: $tap_device"
        fi
    ...
    if [ "$use_cloud_init" = true ]; then
      ...
      # Minimal ARC-mode setup with comprehensive logging
      log() {
          echo \"[\$(date '+%Y-%m-%d %H:%M:%S')] \$1\" | tee -a /var/log/setup-runner.log
      }
      
      log \"=== Minimal GitHub Actions Runner Setup Starting ===\"
      log \"VM Name: \${RUNNER_NAME:-\$(hostname)}\"
      log \"GitHub URL: \${GITHUB_URL:-'not set'}\"
      log \"Runner Labels: \${RUNNER_LABELS:-'not set'}\"
      ...
      for i in {1..60}; do
          ...
          if [ \$i -eq 60 ]; then
              log \"❌ ERROR: Network connectivity timeout after 60 attempts\"
              exit 1
          fi
          log \"⏳ Network not ready, attempt \$i/60...\"
          sleep 2
      done
      ...
      usermod -aG docker runner 2>/dev/null || log \"⚠️  Warning: Failed to add runner to docker group\"
      ...
      if [ -f .runner ]; then
          log \"🧹 Removing existing runner configuration...\"
          sudo -u runner ./config.sh remove --token \"\$RUNNER_TOKEN\" 2>/dev/null || log \"⚠️  Warning: Failed to remove existing config\"
      fi
      ...
      log \"⚙️  Configuring runner with token: \${RUNNER_TOKEN:0:10}...\"
      sudo -u runner ./config.sh \\
          --url \"\$GITHUB_URL\" \\
          --token \"\$RUNNER_TOKEN\" \\
          --name \"\${RUNNER_NAME:-\$(hostname)}\" \\
      ...
      if [ -f .runner ]; then
          log \"📋 Runner details:\"
          cat .runner | jq . 2>/dev/null | sed 's/^/    /' || cat .runner | sed 's/^/    /'
      fi
      
      log \"🚀 Starting GitHub runner service...\"
      systemctl enable github-runner || log \"⚠️  Warning: Failed to enable github-runner service\"
      systemctl start github-runner || {
          log \"❌ ERROR: Failed to start github-runner service\"
          systemctl status github-runner --no-pager | sed 's/^/    /' || true
          exit 1
      ...
      log \"  - Runner Service: \$(systemctl is-active github-runner)\"
      log \"  - Runner Process: \$(pgrep -f 'actions.runner' >/dev/null && echo 'Running' || echo 'Not found')\"
      log \"  - VM IP: \$(ip addr show eth0 | grep 'inet ' | awk '{print \$2}' | cut -d'/' -f1 || echo 'Unknown')\"
      log \"=== Setup script completed ===\""
        elif [ "$docker_mode" = true ]; then
            setup_script_content="#!/bin/bash
      set -euo pipefail
      
      # Minimal setup for non-ARC mode
      log() {
          echo \"[\$(date '+%Y-%m-%d %H:%M:%S')] \$1\" | tee -a /var/log/setup-runner.log
      }
      
      log \"=== Minimal GitHub Actions Runner Setup ===\"
...
# Minimal cloud-init configuration for GitHub Actions runner
# Only SSH keys and runner setup - everything else commented out

hostname: ${runner_name}

# Essential: SSH access
users:
  - name: runner
    ssh_authorized_keys:
      - $(cat ssh_key.pub)
...
# Essential: Environment variables and setup script
write_files:
...
# Essential: Run setup script
runcmd:
  # Basic network setup - let systemd handle DHCP
  - dhclient eth0 || true
  # Run the GitHub Actions runner setup
$(if [ "$arc_mode" = true ]; then echo "  - nohup /usr/local/bin/run-with-env.sh > /var/log/arc-runner.log 2>&1 &"; else echo "  - /usr/local/bin/run-with-env.sh"; fi)
...
EOF
    ...
    if [ -n "$custom_kernel" ]; then
        ...
        # Use only the compiled Ubuntu kernel - no downloading
        local built_kernel="${WORK_DIR}/kernels/vmlinux-${KERNEL_VERSION}-ubuntu24"
        if [ -f "$built_kernel" ]; then
            cp "$built_kernel" "vmlinux"
            print_info "Using compiled Ubuntu kernel: $(basename "$built_kernel")"
        else
            print_error "Compiled Ubuntu kernel not found: $built_kernel"
            print_error "Please ensure the compiled kernel is available at: ${WORK_DIR}/kernels/vmlinux-${KERNEL_VERSION}-ubuntu24"
            exit 1
        fi
    fi
    ...
    # Redirect Firecracker output to capture boot logs for debugging
    firecracker --api-sock "$socket_path" > firecracker-console.log 2>&1 &
    local fc_pid=$!
    echo "$fc_pid" > firecracker.pid
    print_info "Firecracker started with PID $fc_pid, console output: $(pwd)/firecracker-console.log"
    sleep 2
    ...
    if [ -d "instances" ] && [ "$(ls -A instances 2>/dev/null)" ]; then
        for inst in instances/*/info.json; do
            if [ -f "$inst" ]; then
                ...
                local log_file="${vm_dir}/firecracker-console.log"
